...
const utilisateursParDefaut = [
  { login: "admin", motDePasse: "glasspro2024", nom: "Administrateur", role: "admin" }
];

function getUtilisateurs(){
  return JSON.parse(localStorage.getItem("utilisateurs")) || utilisateursParDefaut;
}
...
function connecterUtilisateur(login, motDePasse){
  ...
  const user = users.find(u => u.login === login.trim() && (u.motDePasseHash === hashed || u.motDePasse === motDePasse));
  if(user){
...
async function initFirebase(){
  try {
    ...
    console.log("✅ Realtime Database connectée");
...
async function sauvegarderFirebase(){
  ...
  try {
    ...
    console.log("✅ Sauvegardé sur Realtime Database");
  } catch(e){
